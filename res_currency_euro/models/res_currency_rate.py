...
class ResCurrencyRate(models.Model):
    ...
    original_rate = fields.Float(
        ...
        digits=0,
        group_operator="avg",
        help='The selling exchange rate from CRC to EUR as it is send from BCCR')
    ...
    original_rate_2 = fields.Float(
        ...
        group_operator="avg",
        help='The buying exchange rate from CRC to EUR as it is send from BCCR')
    ...
    def _cron_update(self, first_date=False, last_date=False):
        ...
        _logger.info("=========================================================")
        _logger.info("Executing exchange rate update from 1 CRC = X EUR")
                ...
                while node_index < len(selling_rate_nodes):
                    if selling_rate_nodes[node_index].find("DES_FECHA").text == \
                       buying_rate_nodes[node_index].find("DES_FECHA").text:
                        ...
                        # GET THE CURRENCY ID
                        currency_id = self.env['res.currency'].search([('name', '=', 'EUR')], limit=1)
        ...
        if exchange_source == 'hacienda':
            ...
            if first_date:
                ...
                try:
                    url = 'https://api.hacienda.go.cr/indicadores/tc/euro/'+initial_date+'&h='+end_date
                ...
                if response.status_code in (200,):
                    ...
                    for rate_line in data:
                        ...
                        vals = {}
                        vals['original_rate'] = rate_line['dolares']
                        vals['inverse_company_rate'] = rate_line['colones']
                        ...
                        vals['original_rate_2'] = rate_line['dolares']
                        # vals['inverse_company_rate_2'] = rate_line['colones']
                ...
                if response.status_code in (200,):
                    ...
                    factorEUR = self.env.ref('base.EUR').factorEUR
                    today = datetime.now().strftime('%Y-%m-%d')
                    data = response.json()
                    vals = {}
                    rate = data['euro']['colones']
                    rate = rate+ (rate*(factorEUR/100))
                    vals['original_rate'] = rate
                    vals['inverse_company_rate'] = data['euro']['colones']
                    ...
                    rate2 = data['euro']['colones']
                    rate2 = rate2 + (rate2 * (factorEUR / 100))
                    vals['original_rate_2'] = rate2
                    # vals['inverse_company_rate_2'] = data['euro']['colones']
